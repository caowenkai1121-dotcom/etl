...
@Slf4j
@Component
public class DingTalkChannel implements NotificationDispatcher.NotificationChannel {
    ...
    public void send(String title, String message) {
        if (restTemplate == null) {
            log.warn("RestTemplate未配置，无法发送钉钉通知");
            return;
        }
        if (webhookUrl == null || webhookUrl.isEmpty() || webhookUrl.contains("YOUR_ACCESS_TOKEN")) {
            log.warn("钉钉Webhook URL未配置，无法发送通知");
            return;
        }
